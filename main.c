...
int saia_nodes_menu(void) {
    ...
    switch (choice) {
        case 1: {
            ...
            char *content = string_buffer_to_string(buf);
            file_append(nodes_path, content);
            free(content);
        ...
        case 1: {
            ...
            while (fgets(buffer, sizeof(buffer), stdin)) {
                ...
                if (strstr(buffer, ":")) {
                    ...
                    char entry[512];
                    snprintf(entry, sizeof(entry), "admin:%s\n", buffer);
                    file_append(tokens_path, entry);
                    printf("已添加: admin:%s\n", buffer);
                }
            }
            printf("凭据已添加\n");
...
int main(int argc, char *argv[]) {
    ...
    // 获取当前目录
    if (getcwd(g_config.base_dir, sizeof(g_config.base_dir)) == NULL) {
        strcpy(g_config.base_dir, ".");
    }

    // 初始化配置
    if (config_init(&g_config, g_config.base_dir) != 0) {
        fprintf(stderr, "配置初始化失败\n");
        return 1;
    }

    // 初始化压背控制
    if (backpressure_init(&g_config.backpressure) != 0) {
        fprintf(stderr, "压背控制初始化失败\n");
        return 1;
    }
